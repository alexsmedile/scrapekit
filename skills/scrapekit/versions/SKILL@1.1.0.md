...
### Subagent permissions (required for web-researcher, doc-fetcher, social-scraper)

The scrapekit agents are spawned as subagents and need explicit permission grants to use Bash and WebFetch without prompting. Since the plugin is installed globally, these permissions must go in `~/.claude/settings.json` (project-level settings won't work — the agents run in any project).

If the user reports that a subagent failed with a permissions error, guide them to add this block to the correct settings file:
